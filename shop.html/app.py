...
ADS_FOLDER = os.path.join('static', 'images', 'ads') # New folder for ads
USER_IMAGE_FOLDER = os.path.join('static', 'images', 'users')
ALLOWED_EXTENSIONS = {'png', 'jpg', 'jpeg', 'gif'}
app.config['IMAGE_FOLDER'] = IMAGE_FOLDER
app.config['ADS_FOLDER'] = ADS_FOLDER # New config
USERS_FILE = os.path.join('static', 'users.json')
ADS_FILE = os.path.join('static', 'ads.json') # New ads db file
...
def create_park():
    ...
    try:
        ...
        for i, file_storage in enumerate(uploaded_files):
            ...
            image_path = os.path.join(app.config['IMAGE_FOLDER'], full_filename)
            ...
            lqip_path = os.path.join(app.config['IMAGE_FOLDER'], lqip_filename)
...
def update_park_details(park_id):
    ...
    try:
        ...
        for old_filename in files_to_delete:
            ...
            old_lqip_path = os.path.join(app.config['IMAGE_FOLDER'], old_filename.rsplit('.', 1)[0] + '.lqip.jpg')
        ...
        for item in files_to_save:
            ...
            lqip_path = os.path.join(app.config['IMAGE_FOLDER'], item['filename'].rsplit('.', 1)[0] + '.lqip.jpg')
...
def get_ads():
    ...
    for ad in ads:
        ad['image_url'] = f"/static/images/ads/{ad['image_filename']}"
    return jsonify(ads)
...
def create_ad():
    ...
    image_path = os.path.join(app.config['ADS_FOLDER'], filename)
...
def remove_ad(ad_id):
    ...
    image_path = os.path.join(app.config['ADS_FOLDER'], ad_to_delete['image_filename'])
    if os.path.exists(image_path):
        os.remove(image_path)
...
if __name__ == '__main__':
    ...
    if not os.path.isfile(CONVERSATIONS_FILE):
        with open(CONVERSATIONS_FILE, 'w') as f:
            json.dump({}, f)
    if not os.path.isfile(ADS_FILE):
        with open(ADS_FILE, 'w') as f:
            json.dump([], f)
    if not os.path.isfile(ADS_FILE):
        with open(ADS_FILE, 'w') as f:
