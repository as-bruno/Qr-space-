...
# --- NEW: Lock files to prevent race conditions on JSON writes ---
USERS_LOCK_FILE = os.path.join(PERSISTENT_STORAGE_PATH, 'users.json.lock')
ADS_LOCK_FILE = os.path.join(PERSISTENT_STORAGE_PATH, 'ads.json.lock')
CONVERSATIONS_LOCK_FILE = os.path.join(PERSISTENT_STORAGE_PATH, 'conversations.json.lock')
...
def get_all_users():
    ...
    # Reading does not require a lock
    try:
...
def chat_page():
    ...
    if target_admin_id and target_admin_id != user_id:
        ...
        if target_user: # Ensure the target user exists
            ...
            with FileLock(CONVERSATIONS_LOCK_FILE):
                conversations = get_conversations()
...
def handle_new_message(data):
    ...
    all_users = get_all_users() # --- OPTIMIZATION: Fetch all users only once.
    user_id = session.get('user_id')
    if not user_id:
        return # Not authenticated

    current_user = next((u for u in all_users if u.get('id') == user_id), None)
    if not current_user:
        return
    ...
    else: # The sender is a normal user
        sender_type = 'user'
    ...
    main_admin = next((u for u in all_users if u.get('email') == 'qrbxb70@gmail.com'), None)
    ...
    with FileLock(USERS_LOCK_FILE):
        try:
            ...
            new_user = {
                ...
                "photo": data.get('photo'), "role": "normal"
            }
