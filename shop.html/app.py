...
if SECRET_KEY is None:
    ...
    print("Generating a new secret key. Please set the environment variable FLASK_SECRET_KEY")

app.secret_key = SECRET_KEY

IS_DEBUG_MODE = os.environ.get('FLASK_DEBUG', 'False').lower() in ('true', '1', 't')
if app.secret_key == 'a-very-long-and-random-secret-key-in-dev' and not IS_DEBUG_MODE:
...
def update_park_details(park_id):
    ...
    is_main_admin = current_user.get('email') == os.environ.get('MAIN_ADMIN_EMAIL', 'qrbxb70@gmail.com')
        ...
        if current_user:
            ...
            if current_user.get('role') == 'admin':
                for key, convo_data in conversations.items():
                    messages_json = convo_data.get('messages', '[]')
                    deleted_by_json = convo_data.get('deleted_by', '[]')
                    try:
                        messages = json.loads(messages_json)
                        deleted_by = json.loads(deleted_by_json)
                    except json.JSONDecodeError:
                        continue
...
def get_conversations():
    """Reads all chat conversations from the database."""
    db = get_db()
    convo_rows = db.execute('SELECT * FROM conversations').fetchall()
    return {row['id']: dict(row) for row in convo_rows}

def save_conversations(conversations):
    """Saves the conversations dictionary to the database."""
    db = get_db()
    for convo_id, data in conversations.items():
        db.execute('INSERT OR REPLACE INTO conversations (id, messages, deleted_by) VALUES (?, ?, ?)',
                   (convo_id, json.dumps(data.get('messages', [])), json.dumps(data.get('deleted_by', []))))
    db.commit()
...
def chat_page():
    ...
    if is_support_request or report_merchant_id:
        ...
        main_admin = next((u for u in users if u.get('email') == os.environ.get('MAIN_ADMIN_EMAIL', 'qrbxb70@gmail.com')), None)
        if main_admin:
    ...
    if target_admin_id and target_admin_id != user_id:
        # The key is always user_id-admin_id. The current user is the first part.
        conversation_key = f"{user_id}-{target_admin_id}"
        
        conversations = get_conversations()
        if conversation_key not in conversations: # Create if not exists
            db = get_db()
            db.execute('INSERT OR IGNORE INTO conversations (id, messages, deleted_by) VALUES (?, ?, ?)',
                       (conversation_key, '[]', '[]'))
            db.commit()
    ...
    if current_user.get('role') == 'admin':
        if target_admin_id and target_admin_id != user_id:
            # Admin wants to chat with another admin, create the key and redirect
            convo_key = f"{user_id}-{target_admin_id}"
            return redirect(url_for('admin_chat_dashboard', open_convo=convo_key))
...
def admin_chat_dashboard():
    ...
    for key, convo_data in conversations.items():
        ...
        if is_main_admin or admin_id in [user_part, admin_part]:
            ...
            visible_conversations[key] = messages # Keep this for the template
...
def mark_as_seen():
    ...
    if updated:
        ...
        db.execute('UPDATE conversations SET messages = ? WHERE id = ?',
                   (json.dumps(messages), conversation_key))
        db.commit()
...
def delete_conversation(conversation_id):
    ...
    deleted_by_json = convo_data.get('deleted_by', '[]')
    try:
        deleted_by_list = json.loads(deleted_by_json)
    except json.JSONDecodeError:
        deleted_by_list = []
    ...
    try:
        ...
        if user_part in deleted_by_list and admin_part in deleted_by_list:
            ...
            db.execute('UPDATE conversations SET deleted_by = ? WHERE id = ?', (json.dumps(deleted_by_list), conversation_id))
    except ValueError:
        db.execute('DELETE FROM conversations WHERE id = ?', (conversation_id,))
...
def handle_new_message(data):
    ...
    db = get_db()
    convo_row = db.execute('SELECT messages, deleted_by FROM conversations WHERE id = ?', (conversation_key,)).fetchone()
    
    messages = json.loads(convo_row['messages']) if convo_row else []
    deleted_by = json.loads(convo_row['deleted_by']) if convo_row and convo_row['deleted_by'] else []

    # If the recipient had deleted the chat, this new message "resurrects" it for them.
    recipient_id = target_room
    if recipient_id in deleted_by:
        deleted_by.remove(recipient_id)

    messages.append(new_message)
    
    db.execute('INSERT OR REPLACE INTO conversations (id, messages, deleted_by) VALUES (?, ?, ?)',
               (conversation_key, json.dumps(messages), json.dumps(deleted_by)))
    db.commit()
    ...
    main_admin = next((u for u in get_all_users() if u.get('email') == os.environ.get('MAIN_ADMIN_EMAIL', 'qrbxb70@gmail.com')), None)
